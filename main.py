...
async def cancel_handler_uqests_states(message: types.Message, state: FSMContext):
    ...
    await message.reply('Ви вийшли з режиму створення посту', reply_markup=types.ReplyKeyboardRemove())
...
async def text_handler(message: types.Message, state: FSMContext):
    if len(message.text) > 900:
        ...
        info = """
        Текст посту збережено\n\nВставте посилання для кнопки\n\n---Для виходу введіть  "/cancel"
        """
        async with state.proxy() as data:
            data['text'] = message.text
        await state.set_state(State_admin.btn_link)   
    await message.answer(info)

"""
- STATE: BTN_LINK -> BTN_TEXT
- ABOUT: SET BTN LINK
"""
@dp.message_handler(state=State_admin.btn_link)
async def btn_link_handler(message: types.Message, state: FSMContext):
    info = """
    Посилання для кнопки збережено\n\nВведіть текст кнопки\n\n---Для виходу введіть  "/cancel"
    """
    async with state.proxy() as data:
...
"""
- STATE: BTN_LINK -> BTN_LINK
- ABOUT: INVALID CONTENT TYPE
"""

@dp.message_handler(lambda message: message.text is None, content_types=ContentType.ANY, state=State_admin.text)
async def invalid_btn_link_handler(message: types.Message,  state: FSMContext):
    info = """
    Некоректне повідомлення
...
async def btn_text_handler(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        keyboard.add(InlineKeyboardButton(data['btn_text'], url=data['btn_link']))
